...
class CactiWrapper:
    ...
    def SRAM_estimate_energy(self, interface):
        ...
        #============================================================================
        # Optimization for data reuse, uncomment if want to use
        # need to modify cacti output csv entries to match the required field names
        # 1. Number of banks need to be added as a field to output csv as a field
        # 2. Number of rw ports need to be added as a field to output csv as a field
        #============================================================================
        ...
        # query Cacti
        self.cacti_wrapper_for_SRAM(cacti_exec_dir, tech_node, size_in_bytes, wordsize_in_bytes, n_rw_ports, n_banks)
        with open(csv_file_path) as csv_file:
            reader = csv.DictReader(csv_file)
            row = list(reader)[-1]
            if not action_name == 'idle':
    ...
    def cacti_wrapper_for_SRAM(self, cacti_exec_dir, tech_node, size_in_bytes, wordsize_in_bytes, n_rw_ports, n_banks):
        ...
        temp_output =  open("tmp_output.txt", "wb")
